...
export default async function CapabilitiesPage() {
  ...
  const [capabilitiesResult, historyResult, definitionsResult] = await Promise.all([
    ...
    supabase.from("capability_level_definitions").select("capability_id, level, standard, promotion_statement"),
  ]);
  if (capabilitiesResult.error || historyResult.error || definitionsResult.error) throw new Error("无法读取能力发展状态");
  ...
  return <div className="daily-page">
    <PageHeader title="能力发展" description="所有能力从 L1 开始；晋级需要完成必修里程碑、满足证据门槛，并经正式评估。" />
